...
const PetManagement = () => {
  ...
  if (loading) return (
    <div className="flex justify-center items-center min-h-screen">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-xl font-semibold text-gray-700">
        Loading Pets...
      </motion.div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-gray-800 tracking-tight">Manage Pets</h1>
        <motion.button
      ...
      {pets.length === 0 ? (
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center text-gray-500 text-lg py-12">
          No pets found.
        </motion.p>
      ) : (
          ...
          <table className="w-full bg-white">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold text-gray-600">Rent Price</th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-gray-600">Buy Price</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {pets.map((pet) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-gray-800 font-medium">{pet.name}</td>
                  <td className="px-6 py-4 capitalize text-gray-600">{pet.category}</td>
